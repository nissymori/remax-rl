"""Atari PPO with action-value critic.

Ported from the MinAtar variant in this repository and adapted to the
Atari envpool pipeline used by REPPO.
"""
...
def make_update_fn(config: PPOConfig):
    def _update_step(runner_state):
        ...
        params, opt_state, env_state, last_obs, rng = runner_state
        logits, last_q = network.apply(params, last_obs[: config.num_envs])
        probs = jax.nn.softmax(logits)
        last_val = jnp.sum(probs * last_q, axis=-1)
            ...
            def _update_minibatch(state, batch):
                ...
                def _loss_fn(params, traj_batch, gae, targets):
                    ...
                    q_taken = jnp.take_along_axis(
                        q_values, traj_batch.action[..., None], axis=-1
                    ).squeeze(-1)
                    q_old = jnp.take_along_axis(
                        traj_batch.q_values, traj_batch.action[..., None], axis=-1
                    ).squeeze(-1)
                    q_clipped = q_old + (q_taken - q_old).clip(-config.clip_eps, config.clip_eps)
                    value_losses = jnp.square(q_taken - targets)
                    value_losses_clipped = jnp.square(q_clipped - targets)
...
if __name__ == "__main__":
    ...
    if args.save_model:
        with open(f"{args.env_name}-seed={args.seed}.ppo_q", "wb") as f:
